...
if(is_array($q) && count($q) > 0){
	$err = array();
	$sql = 'UPDATE '.TABLE_ITEMS.' SET `signature` = ?, `file_id` = ?, `rarity` = ?, `weight` = ?, `type` = ?, `subtype` = ?, `unlock_type` = ?, `level` = ?, `value` = ?, `pvp` = ?, `attr1` = ?, `attr2` = ?, `attr3` = ?, `attr_name` = ?, `unlock_id` = ?, `name_de` = ?, `name_en` = ?, `name_es` = ?, `name_fr` = ?, `desc_de` = ?, `desc_en` = ?, `desc_es` = ?, `desc_fr` = ?, `data_de` = ?, `data_en` = ?, `data_es` = ?, `data_fr` = ?, `updated` = ?, `update_time` = ?  WHERE `id` = ?';
	...
	if($stmt = mysqli_prepare($db, $sql)){
		...
		mysqli_stmt_bind_param($stmt, 'sisssssiiissssissssssssssssiii', $signature, $file_id, $rarity, $weight, $type, $subtype, $unlock_type, $level, $value, $pvp, $attr1, $attr2, $attr3, $attr_name, $unlock, $name_de, $name_en, $name_es, $name_fr, $desc_de, $desc_en, $desc_es, $desc_fr, $json_de, $json_en, $json_es, $json_fr, $updated, $update_time, $item_id);
		foreach($q as $i){
